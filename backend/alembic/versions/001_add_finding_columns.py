...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "finding_type",
            sa.Enum("CVE", "Secret", "IaC", "SAST", "License", name="findingtype"),
            nullable=False,
        ),
        ...
        sa.Column(
            "severity",
            sa.Enum(
                "Critical", "High", "Medium", "Low", "Informational", name="severity"
            ),
            nullable=False,
        ),
        sa.Column(
        ...
        sa.Column(
            "suppression_scope",
            sa.Enum("global", "contextual", name="suppressionscope"),
            nullable=True,
        ),
        sa.Column(
            "attestation", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "regression_of", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        "ix_findings_fingerprint_id", "findings", ["fingerprint_id"], unique=True
    )
