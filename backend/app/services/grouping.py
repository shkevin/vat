...
def finding_to_api_dict_with_group_key(f: Finding, *, slim: bool = False) -> dict:
    ...
    Both paths go through ``FindingRead.model_validate`` (Pydantic-fast) but
    the slim variant uses ``to_slim_api_dict`` to drop fields the list-level
    views never read. The deferred columns (description/justification/etc.)
    are on the schema as Optional and excluded from ``model_validate``'s
    field list when ``slim=True`` is requested via the
    ``FindingRead._slim_validate`` shortcut, which reads only the fields the
    slim dict actually emits.
    """
    from app.schemas.finding import FindingRead

    if slim:
        d = FindingRead._slim_from_orm(f).to_slim_api_dict()
    else:
