...
class AuditEvent(Base):
    ...
    parser_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    asset_id: Mapped[Optional[str]] = mapped_column(String(512), nullable=True, index=True)
    finding_id: Mapped[Optional[str]] = mapped_column(String(32), nullable=True, index=True)
    decision_name: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    decision_reason_code: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    decision_confidence: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    ...
    __table_args__ = (
        ...
        Index("ix_audit_events_source_parser_created", "source_id", "parser_id", "created_at"),
    )
