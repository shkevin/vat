...
class Finding(Base):
    ...
    fingerprint_id: Mapped[str] = mapped_column(String(64), unique=True, nullable=False, index=True)
    cve_id: Mapped[str] = mapped_column(String(128), nullable=False)
    severity: Mapped[Severity] = mapped_column(Enum(Severity), nullable=False)
    status: Mapped[Status] = mapped_column(Enum(Status), nullable=False)
    previous_status: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # for revert
    component_base: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
    component: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)  # e.g. "runc 1.1.11"
    image: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)  # e.g. "api-server:latest"
    branch: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)  # git branch for multi-branch repos
    tag: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)  # container image tag
    title: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    source_file_url: Mapped[Optional[str]] = mapped_column(String(2048), nullable=True)  # Direct URL from source (e.g. Aikido)
    file_path: Mapped[Optional[str]] = mapped_column(String(1024), nullable=True)  # For location-based grouping (SAST, Secret, IaC)
    line: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    snippet_masked: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)  # Line preview with secrets masked
    ...
    source: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)  # primary source name
    ...
    sla_due: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # ISO date
    ...
    regression_of: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True)  # [finding_id]
    regression_count: Mapped[int] = mapped_column(Integer, default=0)
    audit: Mapped[list] = mapped_column(JSONB, default=list)  # [{ts, user, action, note}]
    ...
    source_issue_group_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    # Aikido source/workspace id — which Aikido integration (for per-source tracker mapping)
    aikido_source_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    sync_status: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # open | pending_sync | synced | sync_failed
    ...
    first_detected_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
