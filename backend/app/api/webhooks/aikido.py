...
def _fallback_to_vat(data: dict) -> VatFindingSchema | None:
    ...
    try:
        ...
        cve_id = str(_get_nested(issue, "cveId") or _get_nested(issue, "cve_id") or _get_nested(issue, "id") or "unknown")
        comp = str(_get_nested(issue, "component") or _get_nested(issue, "package") or _get_nested(issue, "component_base") or "")
        if comp and _get_nested(issue, "version"):
            comp = f"{comp} {_get_nested(issue, 'version')}"
        sev = str(_get_nested(issue, "severity") or _get_nested(issue, "criticality") or "medium").lower()
        ...
        return VatFindingSchema(
            ...
            title=str(_get_nested(issue, "title") or _get_nested(issue, "name") or cve_id),
...
async def _handle_aikido_webhook(
    request: Request, db: AsyncSession, creds: dict, *, aikido_source_id: str | None = None
) -> Response | dict:
    if not _configured(creds):
        return Response(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content="Aikido not configured")
    ...
    if not verify_replay_timestamp(request, data, max_age_sec=30):
        return Response(status_code=status.HTTP_401_UNAUTHORIZED, content="Replay: timestamp too old or invalid")
    event = data.get("event") or data.get("event_type") or "unknown"
    issue = data.get("issue") or data.get("payload", {}).get("issue") or data
    issue_id = str(_get_nested(issue, "id") or _get_nested(issue, "issue_id") or "")
    ts = data.get("dispatched_at") or data.get("created_at") or data.get("timestamp") or ""
    ...
    if not AikidoAdapter().get_capabilities().supports_inbound_sync:
        ...
        return {"received": True, "event": event, "skipped": "adapter does not support inbound sync"}
    if event in ("issue.closed",):
        ...
        try:
            ...
            source_issue_id = str(getattr(payload, "source_issue_id", None) or "").strip()
        except Exception as e:
            logger.warning("Aikido adapter ingest failed for issue.closed: %s", e)
            issue = data.get("issue") or data.get("payload", {}).get("issue") or data
            cve_id = str(issue.get("cveId") or issue.get("cve_id") or issue.get("id") or "unknown")
            component = str(issue.get("component") or issue.get("package") or issue.get("component_base") or "")
            image = ""
            branch = ""
            tag = ""
            source_issue_id = str(issue.get("id") or issue.get("issue_id") or "").strip()
        async with async_session() as session:
            from app.services.dedup import make_fingerprint
            existing = None
            if source_issue_id:
                existing = await find_finding_by_external_id(session, "Aikido", source_issue_id)
            if not existing:
                fp = make_fingerprint(cve_id, component, image=image, branch=branch, tag=tag)
            ...
            if existing:
                ...
                await record_webhook_processed(session, idempotency_key, "aikido", event, data, {"finding_id": existing.id})
                return {"received": True, "event": event, "finding_id": existing.id, "status": "Resolved"}
            await record_webhook_processed(session, idempotency_key, "aikido", event, data)
    ...
    async with async_session() as session:
        ...
        await record_webhook_processed(session, idempotency_key, "aikido", event, data, {"finding_id": finding.id, "created": created})
        await session.commit()

    return {"received": True, "event": event, "finding_id": finding.id, "created": created}
