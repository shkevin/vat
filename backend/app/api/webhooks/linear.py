...
from app.services.watched_label_inject import handle_issue_label_update, handle_template_reinject
...
async def linear_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not _configured(api_key, team_id):
        return Response(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content="Linear not configured")
    ...
    if is_issue_update:
        ...
        issue_id = str(issue_obj.get("identifier") or issue_obj.get("id") or data_body.get("issueId") or "")
        ...
        if "labelIds" in updated_from or "labels" in updated_from:
            idempotency_key = compute_idempotency_key("linear", "Issue.update.label", issue_id, ts)
            if not await is_duplicate_webhook(db, idempotency_key):
                async with async_session() as session:
                    label_result = await handle_issue_label_update(session, data_body, updated_from)
                    if label_result is not None:
                        await record_webhook_processed(session, idempotency_key, "linear", "Issue.update", data, label_result)
            ...
            if parsed:
                ...
                if not await is_duplicate_webhook(db, idempotency_key):
                    ...
                    async with async_session() as session:
                        desc_result = await apply_vat_parsed_update(
                            session, parsed, issue_id, str(issue_obj.get("id") or ""),
                            idempotency_key, "Issue.update.description", data,
                        )
                ...
                if not await is_duplicate_webhook(db, idempotency_key):
                    ...
                    async with async_session() as session:
                        ...
                        if reinject_result:
                            await record_webhook_processed(session, idempotency_key, "linear", "Issue.update.reinject", data, reinject_result)
                ...
                if not await is_duplicate_webhook(db, idempotency_key):
                    ...
                    async with async_session() as session:
                        reopen_result = await handle_tracker_issue_closed_without_vat(
                            session, adapter, issue_id,
                            str(issue_obj.get("id") or ""),
                            new_state_id, api_key, team_id,
                        )
                        await record_webhook_processed(
                            session, idempotency_key, "linear", "Issue.update.state", data,
                            {"reopened": reopen_result.get("reopened"), "finding_id": reopen_result.get("finding_id")},
                        )
        ...
        return {
            ...
            "template_reinjected": reinject_result.get("reinjected") if reinject_result else None,
    ...
    issue_body_hint = str(issue_obj.get("description") or "") + " " + str(issue_obj.get("title") or "")
    comment_update = adapter.to_vat_comment_update(data, issue_body_hint=issue_body_hint or None)
    ...
    idempotency_key = compute_idempotency_key("linear", action, comment_id or issue_id, comment_ts or issue_uuid)
