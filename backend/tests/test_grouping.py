...
def test_cve_groups_by_package():
    ...
    f1 = _mk_finding(
        ...
        component_base="firefox-esr",
        ecosystem="debian",
    )
    f2 = _mk_finding(
        id="f-2",
        cve_id="CVE-2024-8381",
        component="firefox-esr 115.0",
        component_base="firefox-esr",
        ecosystem="debian",
    )
...
def test_sast_groups_by_rule_id():
    ...
    f1 = _mk_finding(
        ...
        cve_id="python.dangerous-assert",
        rule_id="python.dangerous-assert",
        title="Dangerous assert",
    )
    f2 = _mk_finding(
        id="f-2",
        finding_type=FindingType.SAST,
        cve_id="python.dangerous-assert",
        rule_id="python.dangerous-assert",
        title="Dangerous assert in other.py",
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_secret_different_files_different_groups():
    ...
    f1 = _mk_finding(
        ...
        finding_type=FindingType.Secret,
        cve_id="aikido",
        title="Leaked secret in install.sh",
        rule_id=None,
        secret_type=None,
    )
    f2 = _mk_finding(
        ...
        finding_type=FindingType.Secret,
        cve_id="aikido",
        title="Leaked secret in postlaunch.sh",
        rule_id=None,
        secret_type=None,
    )
    assert get_finding_group_key(f1) != get_finding_group_key(f2)
...
def test_iac_groups_by_rule_id():
    ...
    f2 = _mk_finding(
        ...
        cve_id="AVD-123",
        rule_id="AVD-123",
        title="S3 bucket public in other.tf",
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_npm_yarn_pnpm_same_group_key():
    ...
    f_npm = _mk_finding(
        id="f-1", component="lodash 4.17.21", component_base="lodash", ecosystem="npm"
    )
    f_yarn = _mk_finding(
        id="f-2", component="lodash 4.17.22", component_base="lodash", ecosystem="yarn"
    )
    f_pnpm = _mk_finding(
        id="f-3", component="lodash 4.17.23", component_base="lodash", ecosystem="pnpm"
    )
    assert (
        get_finding_group_key(f_npm)
        == get_finding_group_key(f_yarn)
        == get_finding_group_key(f_pnpm)
    )
...
def test_component_base_fallback_from_component():
    ...
    f = _mk_finding(
        id="f-1", component="lodash 4.17.21", component_base=None, ecosystem="npm"
    )
...
def test_grouping_scoped_within_asset():
    ...
    f1 = _mk_finding(
        id="f-1", component="urllib3 1.0", component_base="urllib3", ecosystem="pypi"
    )
    f2 = _mk_finding(
        id="f-2", component="urllib3 1.0", component_base="urllib3", ecosystem="pypi"
    )
    f1.image = "repo-a"
    f1.branch = "main"
    f2.image = "repo-b"
...
def test_sast_rule_title_strips_extensionless_paths():
    ...
    f1 = _mk_finding(
        id="f-1", finding_type=FindingType.SAST, rule_id="", title="Dangerous assert"
    )
    f2 = _mk_finding(
        id="f-2",
        finding_type=FindingType.SAST,
        rule_id="",
        title="Dangerous assert in Dockerfile",
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_normalize_package_name():
    ...
    assert (
        normalize_package_name("maven", "org.springframework:spring-core")
...
def test_normalize_package_name_maven_fallback():
    ...
    assert (
        normalize_package_name("gradle", "apache hadoop 1.1.1") == "apache hadoop 1.1.1"
    )
...
def test_sca_different_ecosystems_different_groups():
    ...
    f_debian = _mk_finding(
        id="f-1", component="firefox 115", component_base="firefox", ecosystem="debian"
    )
    f_pypi = _mk_finding(
        id="f-2", component="firefox 1.0", component_base="firefox", ecosystem="pypi"
    )
...
def test_sca_pip_pipenv_poetry_use_pypi_package_normalization():
    ...
    f_pip = _mk_finding(
        id="f-1", component="my_pkg 1.0", component_base="my_pkg", ecosystem="pip"
    )
    f_pypi = _mk_finding(
        id="f-2", component="my_pkg 2.0", component_base="my_pkg", ecosystem="pypi"
    )
...
def test_sast_fallback_to_cwe_id_when_no_rule_id():
    ...
    f2 = _mk_finding(
        ...
        rule_id="",
        cwe_id="CWE-89",
        title="SQL injection in other.py",
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_sast_fallback_to_title_when_no_rule_or_cwe():
    ...
    f1 = _mk_finding(
        ...
        rule_id="",
        cwe_id="",
        title="Dangerous assert in foo.py",
    )
    f2 = _mk_finding(
        id="f-2",
        finding_type=FindingType.SAST,
        rule_id="",
        cwe_id="",
        title="Dangerous assert in bar.py",
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_sast_title_strips_at_line_in_path():
    ...
    f1 = _mk_finding(
        ...
        finding_type=FindingType.SAST,
        rule_id="",
        title="SQL injection at line 42 in db",
    )
    f2 = _mk_finding(
        id="f-2", finding_type=FindingType.SAST, rule_id="", title="SQL injection"
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_sast_title_at_line_in_path_with_extension_partial_strip():
    ...
    f1 = _mk_finding(
        ...
        finding_type=FindingType.SAST,
        rule_id="",
        title="SQL injection at line 42 in db.py",
    )
    f2 = _mk_finding(
        id="f-2", finding_type=FindingType.SAST, rule_id="", title="SQL injection"
    )
...
def test_iac_fallback_to_title_when_no_rule_id():
    ...
    f1 = _mk_finding(
        ...
        finding_type=FindingType.IaC,
        rule_id="",
        title="S3 bucket public in main.tf",
    )
    f2 = _mk_finding(
        id="f-2",
        finding_type=FindingType.IaC,
...
def test_iac_fallback_to_id_when_all_empty():
    ...
    f = _mk_finding(
        id="f-iac-xyz", finding_type=FindingType.IaC, cve_id="", rule_id="", title=""
    )
...
def test_iac_title_strips_makefile_dockerfile():
    ...
    f2 = _mk_finding(
        id="f-2", finding_type=FindingType.IaC, rule_id="", title="Hardcoded secret"
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_secret_title_preserves_path_when_no_type_or_rule():
    ...
    f = _mk_finding(
        id="f-1",
        finding_type=FindingType.Secret,
        title="Leaked secret in local_store.py",
        rule_id=None,
        secret_type=None,
    )
    key = get_finding_group_key(f)
...
def test_asset_empty_produces_double_pipe():
    ...
    f = _mk_finding(
        id="f-1", component="pkg 1.0", component_base="pkg", ecosystem="npm"
    )
...
def test_asset_tag_only():
    ...
    f = _mk_finding(
        id="f-1", component="pkg 1.0", component_base="pkg", ecosystem="npm", tag="v1.0"
    )
...
def test_sast_title_strips_extensionless_gitignore_dockerignore():
    ...
    f1 = _mk_finding(
        ...
        finding_type=FindingType.SAST,
        rule_id="",
        title="Hardcoded value in .gitignore",
    )
    f2 = _mk_finding(
        id="f-2", finding_type=FindingType.SAST, rule_id="", title="Hardcoded value"
    )
    assert get_finding_group_key(f1) == get_finding_group_key(f2)
...
def test_whitespace_in_title_normalized():
    ...
    f1 = _mk_finding(
        id="f-1", finding_type=FindingType.SAST, rule_id="", title="  SQL Injection  "
    )
    f2 = _mk_finding(
        id="f-2", finding_type=FindingType.SAST, rule_id="", title="sql injection"
    )
...
def test_sca_cve_id_normalized_lowercase():
    ...
    f = _mk_finding(
        id="f-1", cve_id="CVE-2024-1234", component=None, component_base=None
    )
    assert get_finding_group_key(f) == "cve:cve-2024-1234#||"
...
def test_secret_title_case_normalized():
    ...
    f = _mk_finding(
        id="f-1",
        finding_type=FindingType.Secret,
        title="Leaked Secret In Config.Env",
        rule_id=None,
        secret_type=None,
    )
...
def test_component_base_extraction_name_space_version():
    ...
    f = _mk_finding(
        id="f-1",
        component="vllm 0.8.5.post1+cpu",
        component_base=None,
        ecosystem="pypi",
    )
    key = get_finding_group_key(f)
