...
import { formatFileLocation, getRepoFileUrl, parseFileLocation } from "@/lib/repoFileUrl";
...
import { daysLeft, displayTitle, displaySourceName, fmtDt as fmtDtUtil, now } from "@/lib/utils";
...
export function DetailPanel({
  ...
  const [attApprover, setAttApprover] = useState(finding.attestation?.approver ?? "");
  const [attTitle, setAttTitle] = useState(finding.attestation?.approverTitle ?? "");
  const [attExpiry, setAttExpiry] = useState(finding.attestation?.expiresAt ?? "");
  ...
  const [syncToTrackerMessage, setSyncToTrackerMessage] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<"details" | "decision" | "history">("details");
  ...
  useEffect(() => {
    ...
    setSuppScope((finding.suppressionScope as "global" | "contextual") ?? "contextual");
    const needsTriage = !["Resolved", "False Positive", "Suppressed", "Not Applicable", "Approved", "Duplicate", "Mitigated", "Risk Accepted"].includes(finding.status);
  ...
  const allTrackers = trackers?.length ? trackers : (tracker?.name ? [tracker] : []);
  const trackerLinks = (finding.externalLinks ?? []).filter((l) => l.kind === "tracker");
  ...
  const doUpdate = async (status: string, extra: Partial<Finding> = {}) => {
    ...
    const updated: Finding = {
      ...
      audit: [
        ...(finding.audit ?? []),
        { ts: now(), user: "reviewer", action: `Status → ${status}`, note: rvNote || null },
      ],
    };
    try {
  ...
  const doSuppress = () => doUpdate("Suppressed", { suppressionScope: suppScope });
    ...
    >
      {finding.archived && (
        <div className="detail-panel-alert detail-panel-alert-archived">
          <span>🗄</span>
          <div style={{ flex: 1 }}>
            <span style={{ ...mono, fontSize: 10, fontWeight: 700, color: "var(--app-accent)" }}>ARCHIVED</span>
            {finding.archivedAt && (
              <span style={{ ...sans, fontSize: 11, color: "var(--app-accent)", marginLeft: 8 }}>
                {fmtDtUtil(finding.archivedAt)}
              </span>
            )}
            {finding.archivedReason && (
              <div style={{ ...sans, fontSize: 11, color: "var(--app-accent)", marginTop: 2, fontStyle: "italic" }}>
                &quot;{finding.archivedReason}&quot;
              </div>
            )}
          </div>
      ...
      {finding.status === "Reopened" && (
        <div className="detail-panel-alert detail-panel-alert-regression">
          ...
          <div style={{ flex: 1 }}>
            <span style={{ ...mono, fontSize: 10, fontWeight: 700, color: "var(--app-warning)" }}>
              REGRESSION — #{finding.regressionCount ?? 0}
            </span>
            <span style={{ ...sans, fontSize: 11, color: "var(--app-warning)", marginLeft: 8 }}>
              This finding was previously resolved and has re-appeared in a scan.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="detail-panel-hero">
          <div className="detail-panel-hero-content">
            <div className="detail-panel-badges">
              ...
              <span style={{ ...mono, fontSize: 13, fontWeight: 700, color: "var(--app-accent)" }}>{finding.cveId}</span>
              <SevTag sev={finding.severity} />
              <StTag status={finding.status} />
              {(finding.sources?.length ?? 0) >= 1 &&
                finding.sources?.slice(0, 2).map((s, i) => (
            ...
            <div className="detail-panel-meta">
              {repoFileUrl && finding.component ? (
                <>
                  <a href={repoFileUrl} target="_blank" rel="noopener noreferrer" style={{ color: "var(--app-accent)", textDecoration: "none" }}>
                    {finding.component}
                  </a>
                  {" · "}
                  {finding.image}
                ...
                <>
                  {finding.component ?? "—"} · {finding.image ?? finding.tag ?? "—"}
                </>
              )}
              {finding.cvss && finding.cvss !== "—" ? ` · CVSS ${finding.cvss}` : ""}
              {finding.epss && finding.epss !== "—" ? ` · EPSS ${finding.epss}` : ""}
            </div>
          </div>
          <button type="button" onClick={onClose} aria-label="Close panel" className="detail-panel-close">
            ×
          </button>
        </div>
      </div>

      <nav role="tablist" aria-label="Finding detail sections" className="detail-panel-tabs">
        {[
      ...
      <div className="detail-panel-body">
        ...
        <div className="detail-panel-risk-bar">
          ...
          {finding.slaDue && (
            <div className={`detail-panel-metric-pill ${d !== null && d < 0 ? "sla-urgent" : d !== null && d < 7 ? "sla-warning" : "sla-ok"}`}>
              <span className="metric-label">SLA</span>
              <span className="metric-value">{d !== null ? `${d}d` : "—"}</span>
            </div>
          )}
          {(finding.owner?.trim() ?? "") !== "" && (
            <div className="detail-panel-metric-pill">
              <span className="metric-label">Owner</span>
              <span className="metric-value" style={{ fontSize: 12 }}>{finding.owner}</span>
            </div>
          )}
        </div>
        ...
        <div className="detail-panel-section">
          ...
          {(() => {
            ...
            const locRow: [string, React.ReactNode] | null = locStr
              ? [
                  "Location",
                  <div key="loc" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    {repoFileUrl ? (
                      <a href={repoFileUrl} target="_blank" rel="noopener noreferrer" className="detail-panel-kv-item value" style={{ display: "inline" }}>
                        {locStr}
                      </a>
                    ) : (
                      <span className="detail-panel-kv-item value">{locStr}</span>
                    )}
                    <button
                      type="button"
                      onClick={() => {
            ...
            const sameGroup = groupFindings && allFindings.length > 0
              ? allFindings.filter((f) => getFindingGroupKey(f) === groupKey)
              : [];
            ...
            const showScopeLinePreview = finding.snippetMasked && !hasSubissues && (!hasSources || isInstanceView);
            const snippetRow: [string, React.ReactNode] | null =
              showScopeLinePreview
                ...
                : null;
            const allScopeRows = snippetRow ? [...scopeRows, snippetRow] : scopeRows;
            return (
        ...
        {groupFindings && selectedSourceIndex == null && allFindings.length > 0 && (() => {
          const groupKey = getFindingGroupKey(finding);
          const sameGroup = allFindings.filter((f) => getFindingGroupKey(f) === groupKey);
          ...
          const getContextLabel = (f: Finding): string => {
            ...
            const tag = f.tag ?? (img.includes(":") ? img.split(":")[1] : undefined);
          ...
          const sevIndex = (f: Finding) => {
            const i = SEV_ORDER.indexOf(f.severity as (typeof SEV_ORDER)[number]);
            return i >= 0 ? i : 999;
          };
          const byContextSorted = Array.from(byContext.entries())
            .map(([ctxLabel, list]) => [ctxLabel, [...list].sort((a, b) => sevIndex(a) - sevIndex(b))] as const)
            .sort(([, a], [, b]) => {
              const worstA = a.reduce((w, f) => (sevIndex(f) < sevIndex(w) ? f : w));
              const worstB = b.reduce((w, f) => (sevIndex(f) < sevIndex(w) ? f : w));
          ...
          return (
            <div className="detail-panel-section">
              ...
              <p className="detail-panel-prose" style={{ marginBottom: 16 }}>
                Same finding across {byContext.size} context{byContext.size !== 1 ? "s" : ""}. Each instance may require a separate fix.
              </p>
              {byContextSorted.map(([ctxLabel, list]) => (
                ...
                >
                  ...
                  <div style={{ padding: 0 }}>
                    {list.map((f) => {
                      const loc = f.filePath ?? parseFileLocation(f)?.filePath ?? f.component ?? "—";
                      const fileUrl = getRepoFileUrl(f, repoBaseUrl, repoUrlType);
                      const lineInfo = f.line != null ? `Line ${f.line}` : null;
                      const locDisplay = lineInfo && loc !== "—" ? `${lineInfo} in ${loc}` : lineInfo || loc;
                      return (
                        <div
                          key={f.id}
                          style={{
                            ...
                            gap: 8,
                            padding: "12px 14px",
                            borderBottom: list.indexOf(f) < list.length - 1 ? "1px solid var(--app-border-subtle)" : "none",
                          }}
                        >
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                              <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: "var(--app-accent)" }}>
                                {f.cveId}
                              </span>
                              <SevTag sev={f.severity} />
                            </div>
                            <div style={{ ...mono, fontSize: 10, color: "var(--app-muted)", marginTop: 4 }}>
                              {fileUrl && loc !== "—" ? (
                                <a
                                  href={fileUrl}
                                  target="_blank"
                            ...
                            {f.snippetMasked && (
                              <pre className="detail-panel-snippet" style={{ marginTop: 8 }}>
                                {f.snippetMasked}
                              </pre>
                            )}
                          </div>
        ...
        {finding.findingType === "Secret" && (
          <div className="detail-panel-section">
            <div className="detail-panel-card detail-panel-alert-secret" style={{ border: "1px solid color-mix(in srgb, var(--app-danger) 30%, transparent)" }}>
              <div className="detail-panel-alert-secret-title">
                🔑 SECRET FINDING — SLA: 24 HOURS
              </div>
              <p className="detail-panel-alert-secret-body">
                Leaked secrets require immediate credential rotation regardless of whether the secret has been accessed.
              </p>
            </div>
          </div>
        )}
        ...
        <CollapsibleSection title="Tracking & Sources" defaultOpen>
          {(finding.sources?.length ?? 0) >= 1 && (() => {
          const sourcesToShow = selectedSourceIndex != null && finding.sources?.length
            ? [finding.sources[selectedSourceIndex]].filter(Boolean)
            : finding.sources ?? [];
          if (sourcesToShow.length === 0) return null;
          return (
          <div className="detail-panel-section">
            <H>Source{sourcesToShow.length > 1 ? ` Attribution (${sourcesToShow.length} sources)` : ""}</H>
            {showAdminActions && onOverrideFingerprint && sourcesToShow.length > 1 && (
              <div
                style={{
                  marginTop: 8,
                  padding: "8px 10px",
              ...
              >
                <div style={{ ...sans, fontSize: 11, color: "var(--app-muted)", marginBottom: 6 }}>
                  Incorrectly merged? Override fingerprint to prevent future dedup with this finding.
                </div>
                <Btn
                  size="sm"
                  variant="ghost"
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sourcesToShow.map((s, i) => {
                const sourceLink = finding.externalLinks?.find(
                  (l) => l.kind === "source" && l.adapterKey?.toLowerCase() === s.name?.toLowerCase()
                );
                const sourceUrl = sourceLink?.url;
                const displayName = displaySourceName(s.name) || s.name || "";
                const locStr = formatFileLocation(finding) ?? finding.filePath ?? finding.component ?? "—";
                return (
                  <div
                    key={i}
                    style={{
                  ...
                  >
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <div style={{ ...mono, fontSize: 10, color: "var(--app-muted)" }}>
                        {repoFileUrl && locStr !== "—" ? (
                          <a
                            href={repoFileUrl}
                            target="_blank"
        ...
        <div className="detail-panel-section">
          <H>Tracking</H>
          <div className="detail-panel-card">
        <div className="detail-panel-kv-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
          {(() => {
            const baseRows: Array<[string, string, string | null]> = [
              ["Owner", finding.owner ?? "—", null],
              ["Team", finding.team ?? "—", null],
              ["SLA Due", finding.slaDue ? `${fmtDtUtil(finding.slaDue)} (${d !== null ? `${d}d` : "—"})` : "—", null],
            ];
            const trackerRows: Array<[string, string, string | null]> =
              trackerLinks.length > 0
                ? trackerLinks.map((link) => {
                    const t =
                      allTrackers.find((x) => x.id === link.adapterKey) ??
                      (link.adapterKey === "linear" || !link.adapterKey ? tracker : allTrackers.find((x) => (x.type || "linear") === "linear")) ??
                      null;
                    const label = t?.name ?? link.adapterKey ?? "Tracker";
                    const issueId = link.issueId ?? "—";
                    const href =
                      link.url ??
                      (t?.baseUrl && issueId !== "—" && !issueId.startsWith("http")
                        ? `${String(t.baseUrl).replace(/\/$/, "")}/${issueId}`
                        : issueId.startsWith("http")
                          ? issueId
                          : null);
                    return [label, issueId, href];
                  })
                : [[tName, finding.trackerId ?? "—", tracker?.baseUrl && finding.trackerId ? `${String(tracker.baseUrl).replace(/\/$/, "")}/${finding.trackerId}` : null]];
            const tailRows: Array<[string, string, string | null]> = [
              ["Control Ref", finding.controlRef ?? "—", null],
              ["Imported", fmtDtUtil(finding.created), null],
            ];
            return [...baseRows, ...trackerRows, ...tailRows].map(([k, v, href], idx) => (
              <div key={idx} className="detail-panel-kv-item">
                <label>{k}</label>
                <div className="value" style={{ ...mono, fontSize: 12 }}>
                  {v !== "—" && href ? (
                    <a
            ...
            <div style={{ marginTop: 10, marginBottom: 10 }}>
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await syncFindingToTracker(finding.id, { token });
                    setSyncToTrackerMessage(res.enqueued ? res.message : res.message);
                    if (res.enqueued) onUpdate({ ...finding, syncStatus: "pending_sync" });
                  } catch (e) {
                    setSyncToTrackerMessage(e instanceof Error ? e.message : "Sync failed");
                  } finally {
                ...
                style={{
                  ...mono,
                  padding: "6px 12px",
                  background: syncToTrackerLoading ? "var(--app-border)" : "var(--app-accent)",
                  border: "none",
                  borderRadius: 4,
                  color: "var(--app-fg)",
                  cursor: syncToTrackerLoading ? "not-allowed" : "pointer",
                  fontSize: 10,
                  fontWeight: 600,
              ...
              {syncToTrackerMessage && (
                <span
                  style={{
                    ...
                    fontSize: 11,
                    marginLeft: 10,
                    color: syncToTrackerMessage.toLowerCase().includes("enqueued")
                      ? "var(--app-success)"
                      : syncToTrackerMessage.toLowerCase().includes("not configured") || syncToTrackerMessage.toLowerCase().includes("not found")
                        ? "var(--app-danger)"
                        : "var(--app-muted)",
                  }}
                >
              ...
              {priorFindings.map((p) => (
                <div
                  ...
                  style={{
                    ...
                    fontSize: 10,
                    color: "var(--app-muted)",
                    marginBottom: priorFindings.indexOf(p) < priorFindings.length - 1 ? 6 : 0,
                  }}
                >
                  <span style={{ color: "var(--app-accent)", fontWeight: 600 }}>{p.cveId}</span>
        ...
        {((finding.justification || finding.compensatingControls) && !canAct) && (
          <div className="detail-panel-section">
            <H>Resolution</H>
            <div className="detail-panel-card">
              {finding.justification && (
                <>
                  <div style={{ ...mono, fontSize: 9, color: "var(--app-muted)", textTransform: "uppercase", marginBottom: 4 }}>
                    Justification{finding.trackerComment ? " (from tracker)" : ""}
                  </div>
                  <p style={{ ...sans, fontSize: 12, color: "var(--app-muted)", margin: 0, lineHeight: 1.65 }}>
                    {finding.justification}
                  </p>
                </>
              )}
              {finding.compensatingControls && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--app-border-subtle)" }}>
                  <div style={{ ...mono, fontSize: 9, color: "var(--app-muted)", textTransform: "uppercase", marginBottom: 4 }}>
                    Compensating Controls
                  </div>
                  <p style={{ ...sans, fontSize: 12, color: "var(--app-muted)", margin: 0 }}>
                    {finding.compensatingControls}
                  </p>
                </div>
              )}
            ...
            >
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[
                  ...
                  ["Approved At", fmtDtUtil(finding.attestation.approvedAt)],
                  ...
                  <div key={String(k)}>
                    <div style={{ ...mono, fontSize: 9, color: "var(--app-muted)", textTransform: "uppercase", marginBottom: 2 }}>
                      {k}
                    </div>
                    <div style={{ ...mono, fontSize: 11, color: "var(--app-accent)" }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === "decision" && (
          <div id="panel-decision" role="tabpanel" aria-labelledby="tab-decision">
          ...
          <div className="detail-panel-reviewer-notes">
            ...
            <div className="detail-panel-save-notes-btn-wrap">
              <Btn
                size="sm"
                variant="secondary"
                disabled={savingNotes || rvNote === (finding.reviewerNote ?? "")}
                onClick={doSaveNotes}
              >
                {savingNotes ? "…" : "Save"}
              </Btn>
        ...
        {canEdit && canAct && (
          <>
            <div className="detail-panel-form-card">
              <div className="detail-panel-form-card-title">Justification & Controls</div>
              <Field
                label="Justification"
                value={jus}
                onChange={setJus}
            ...
            <div className="detail-panel-form-card">
              <div className="detail-panel-form-card-title">Risk Acceptance Attestation</div>
            ...
            <div className="detail-panel-form-card">
              <div className="detail-panel-form-card-title">Suppression Type</div>
              <div className="detail-panel-suppression-options">
                {[
                  { val: "global" as const, label: "🌐 False Positive", desc: "Scanner is wrong. Suppress everywhere." },
                  { val: "contextual" as const, label: "📍 Contextual", desc: "Real vuln, not exploitable here." },
                ].map((opt) => (
                  <div
                    key={opt.val}
                    role="button"
                    ...
                    onKeyDown={(e) => canAct && (e.key === "Enter" || e.key === " ") && setSuppScope(opt.val)}
                    className={`detail-panel-suppression-option ${suppScope === opt.val ? "selected" : ""}`}
                    style={{ cursor: canAct ? "pointer" : "not-allowed", opacity: canAct ? 1 : 0.6 }}
                  >
                    <div className="detail-panel-suppression-option-label">
                      {opt.label}
                    </div>
                    <div className="detail-panel-suppression-option-desc">{opt.desc}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="detail-panel-action-group">
              <div className="detail-panel-action-group-title">Primary Actions</div>
              <div className="detail-panel-action-row">
                <Btn variant="approve" onClick={() => doUpdate("Approved")} disabled={saving || !canAct}>
                  ✓ Approve
                </Btn>
                <Btn variant="reject" onClick={() => doUpdate("Rejected")} disabled={saving || !canAct}>
                  ✗ Reject
                </Btn>
                <Btn variant="purple" onClick={doRiskAccept} disabled={saving || !canAct || !attApprover.trim()}>
                  ⚠ Accept Risk
                </Btn>
              </div>
            </div>
            <div className="detail-panel-action-group">
              <div className="detail-panel-action-group-title">Resolve & Defer</div>
              <div className="detail-panel-action-row">
                <Btn variant="secondary" onClick={doFP} disabled={saving || !canAct}>
                  🌐 False Positive
                </Btn>
                <Btn variant="secondary" onClick={doSuppress} disabled={saving || !canAct}>
                  📍 Suppress
                </Btn>
                <Btn variant="secondary" onClick={() => doUpdate("Not Applicable")} disabled={saving || !canAct}>
                  ⊘ Not Applicable
                </Btn>
                <Btn variant="secondary" onClick={() => doUpdate("Duplicate")} disabled={saving || !canAct}>
                  ⧉ Duplicate
                </Btn>
                <Btn variant="secondary" onClick={() => doUpdate("Resolved")} disabled={saving || !canAct}>
                  ✓ Resolved
                </Btn>
                <Btn variant="secondary" onClick={() => doUpdate("Mitigated")} disabled={saving || !canAct}>
                  ◐ Mitigated
                </Btn>
              </div>
            </div>
          ...
          <>
            ...
            {!showRevert ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Btn size="sm" variant="ghost" onClick={() => setShowRevert(true)}>
              ...
              >
                <div style={{ ...sans, fontSize: 12, color: "var(--app-warning)", marginBottom: 10 }}>
                  Revert <StTag status={finding.status} /> → <StTag status={finding.previousStatus} />
                </div>
                <Field
                  label="Reason *"
                  value={revertReason}
                ...
                <div style={{ display: "flex", gap: 7, marginTop: 10 }}>
                  ...
                  <Btn variant="ghost" onClick={() => { setShowRevert(false); setRevertReason(""); }}>
                    Cancel
                  </Btn>
                </div>
              </div>
        ...
        {showAdminActions && !finding.archived ? (
          <>
            ...
            {!showArchiveForm ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Btn size="sm" variant="secondary" onClick={() => setShowArchiveForm(true)}>
                  🗄 Archive
                </Btn>
                <span className="detail-panel-archive-hint">
                  Never deleted — permanently retained, hidden from active list.
                </span>
              </div>
            ) : (
              <div
              ...
              >
                ...
                <div style={{ display: "flex", gap: 7, marginTop: 10 }}>
                  ...
                  <Btn variant="ghost" onClick={() => { setShowArchiveForm(false); setArchiveReason(""); }}>
                    Cancel
                  </Btn>
                </div>
              </div>
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <span style={{ ...sans, fontSize: 12, color: "var(--app-accent)" }}>Archived — permanently retained.</span>
              {showAdminActions && (
                <Btn
                  size="sm"
                  variant="ghost"
        ...
        <div className="detail-panel-section">
          ...
          <div className="detail-panel-card">
            {[...(finding.audit ?? [])].reverse().map((e, i) => {
              const isStatusChange = /Status\s*→|status\s*→/i.test(e.action);
              return (
                <div
                  key={i}
                  className={`detail-panel-timeline-item${isStatusChange ? " status-change" : ""}`}
                >
                  <div className="detail-panel-timeline-marker" />
                  <div className="detail-panel-timeline-content">
                    <div className="timeline-meta">
                      <span style={{ color: "var(--app-accent)", fontWeight: 600 }}>{e.user}</span>
                      <span style={{ margin: "0 6px" }}>·</span>
                      {fmtDtUtil(e.ts)}
                    </div>
                    <div className="timeline-action" style={{ ...sans, fontSize: 13 }}>
                      {e.action}
                    </div>
                    {e.note && (
                      <div style={{ ...sans, fontSize: 12, color: "var(--app-muted)", fontStyle: "italic", marginTop: 6 }}>
                        &quot;{e.note}&quot;
                      </div>
                    )}
                  </div>
