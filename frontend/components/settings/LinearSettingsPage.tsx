  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      if (res.dispatched) {
        ...
        if (res.linked != null && res.linked > 0)
          parts.push(`Linked ${res.linked} findings.`);
        if (res.reset != null && res.reset > 0)
          parts.push(`Retrying ${res.reset} failed events.`);
        if (res.processed != null && res.processed > 0)
          parts.push(`Processed ${res.processed} events.`);
        if (res.backfill_enqueued != null && res.backfill_enqueued > 0)
          parts.push(`Enqueued ${res.backfill_enqueued} for sync.`);
      }
      setSyncResult(parts.length > 0 ? parts.join(" ") : "Sync complete.");
    } catch (e) {
      setSyncError(
        e instanceof Error ? e.message : "Failed to sync with Linear",
      );
    } finally {
      setSyncing(false);
    }
        ...
        <>
          ...
          >
            ...
            <span style={{ ...mono, fontSize: 9, color: "var(--app-muted)" }}>
              VAT pulls linked tasks from Aikido; canvas shows Aikido Tracker
              node
            </span>
          </div>
          ...
          {loading ? (
            ...
            >
              ...
              >
                VAT creates issues in Linear and syncs [VAT] comments
                bidirectionally. <strong>Webhook</strong> (real-time): register
                the webhook URL in Linear and enter the secret below — polling
                is disabled. <strong>API polling</strong>: when no webhook
                secret is set, VAT polls every few minutes. A reconciliation job
                runs every 6 hours (configurable) to catch missed webhooks. For
                watched label auto-inject, subscribe to <strong>Issue</strong> →{" "}
                <strong>Update</strong> in addition to <strong>Comment</strong>{" "}
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <select
                  ...
                  onChange={async (e) => {
                    const next = {
                      ...tracker,
                      pushMode: e.target.value as "groups" | "instances",
                    };
                    await onTrackerChange?.(next);
                  }}
                  style={{
                ...
                >
                  <option value="groups">
                    Groups — one ticket per VAT group (backend-calculated)
                  </option>
                  <option value="instances">
                    Instances — one ticket per finding
                  </option>
                </select>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={
                      status.teamIdConfigured
                        ? "•••••••• (leave blank to keep)"
                        : "Enter Linear team ID"
                    }
                    autoComplete="off"
                    style={{
                      ...mono,
              ...
              {status.apiKeyConfigured && status.teamIdConfigured && (
                ...
                >
                  ...
                  >
                    Sync states between VAT and Linear: link existing issues,
                    create new ones, retry failed events, and pull [VAT] updates
                    from Linear (when polling is enabled).
                  </div>
                  <button
                    onClick={handleSync}
                    disabled={syncing}
...
function IssueTemplateField({
  ...
  return (
    <div>
      ...
      >
        Injected into new Linear issues. Placeholders:{" "}
        <code
          style={{ ...mono, color: "var(--app-muted)" }}
        >{`{finding_id}`}</code>
        ,{" "}
        <code
          style={{ ...mono, color: "var(--app-muted)" }}
        >{`{file_path}`}</code>
        , <code style={{ ...mono, color: "var(--app-muted)" }}>{`{line}`}</code>
        ,{" "}
        <code
          style={{ ...mono, color: "var(--app-muted)" }}
        >{`{source_file_url}`}</code>{" "}
        (Aikido code link),{" "}
        <code
          style={{ ...mono, color: "var(--app-muted)" }}
        >{`{source_issue_url}`}</code>{" "}
        (link to finding in source).
      </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                Edit the template injected into new Linear issues. Use{" "}
                <code
                  style={{ ...mono, color: "var(--app-accent)" }}
                >{`{cve_id}`}</code>{" "}
                as placeholder.
              </div>
            </div>
    ...
    return (
      <div style={{ marginTop: 24 }}>
        ...
        >
          ...
          >
            Base URL: {tracker.baseUrl}
          </div>
          <div style={{ ...mono, fontSize: 11, color: "var(--app-muted)" }}>
            Comment prefix: {tracker.commentPrefix}
          </div>
        </div>
      </div>
    );
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setEdit((p) => ({
                    ...p,
                    type: t,
                    baseUrl: TRACKER_URL_PRESETS[t] ?? p.baseUrl,
                  }));
                }}
                style={{
                  ...mono,
          ...
          <div>
            ...
            <input
              value={edit.baseUrl}
              onChange={(e) =>
                setEdit((p) => ({ ...p, baseUrl: e.target.value }))
              }
              style={{
                ...mono,
                width: "100%",
          ...
          >
            <div>
              ...
              <input
                value={edit.icon}
                onChange={(e) =>
                  setEdit((p) => ({ ...p, icon: e.target.value }))
                }
                placeholder="◈"
                style={{
                  ...mono,
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEdit((p) => ({ ...p, commentPrefix: e.target.value }))
                }
                placeholder="[VAT]"
                style={{
                  ...mono,
...
function LabelsConfigForm({
  ...
  return (
    <div style={{ marginTop: 24 }}>
      ...
      >
        These labels are applied when VAT creates new Linear issues. If a label
        does not exist in your Linear team, VAT will auto-create it (requires
        API key with Write or Admin permission).
      </div>
      <div
        style={{
          display: "flex",
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditState((p) => ({ ...p, color: e.target.value }))
                    }
                    style={{
                      width: "100%",
                      height: 32,
