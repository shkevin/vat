...
export function getReportFilterConfig(
  ...
  if (assetTypes.length === 0 && hasIssues)
    assetTypes.push(...REPORT_ASSET_TYPES);
...
export function buildReportFilterBarStructure(
  ...
  const sevOptions = filterConfig.severities
    .map(
      (s) =>
        `<label class="report-filter-option" data-filter="severity" data-value="${esc(
          s,
        )}"><input type="checkbox" value="${esc(
          s,
        )}" data-filter="severity" checked> <span class="report-filter-option-label">${esc(
  ...
  const assetOptions = filterConfig.assets
    .map(
      (a) =>
        `<label class="report-filter-option" data-filter="asset" data-value="${esc(
          a,
        )}"><input type="checkbox" value="${esc(
          a,
        )}" data-filter="asset"> <span class="report-filter-option-label" title="${esc(
          a,
        )}">${esc(
  ...
  const assetTypeOptions = filterConfig.assetTypes
    .map(
      (t) =>
        `<label class="report-filter-option" data-filter="assetType" data-value="${esc(
          t,
        )}"><input type="checkbox" value="${esc(
          t,
        )}" data-filter="assetType" checked> <span class="report-filter-option-label">${esc(
          t,
        )}</span><span class="report-filter-count" data-count-for="${esc(
          t,
        )}"></span></label>`,
  ...
  const branchOptions = filterConfig.branches
    .map(
      (b) =>
        `<label class="report-filter-option" data-filter="branch" data-value="${esc(
          b,
        )}"><input type="checkbox" value="${esc(
          b,
        )}" data-filter="branch"> <span class="report-filter-option-label">${esc(
          b,
        )}</span><span class="report-filter-count" data-count-for="${esc(
          b,
        )}"></span></label>`,
  ...
  return `<div${idAttr} class="report-filter-bar${
    inline ? " report-filter-bar-inline" : ""
  }" style="border-color:${borderColor};color:${mutedColor}">
  <div class="report-filter-facets">
    ...
    ${
      hasAssetTypes
        ? `<div class="report-filter-dd" data-dd="assetType">
    ...
    ${
      hasAssets
        ? `<div class="report-filter-dd" data-dd="asset">
      ...
      <div class="report-filter-panel" data-panel="asset">
        ${
          hasSearchAssets
            ? `<input type="search" class="report-filter-search" placeholder="Search assets…" data-search="asset" autocomplete="off">`
            : ""
        }
        <div class="report-filter-panel-inner">
          ${assetOptions}
    ...
    ${
      hasBranches
        ? `<div class="report-filter-dd" data-dd="branch">
      ...
      <div class="report-filter-panel" data-panel="branch">
        ${
          hasSearchBranches
            ? `<input type="search" class="report-filter-search" placeholder="Search branches…" data-search="branch" autocomplete="off">`
            : ""
        }
        <div class="report-filter-panel-inner">
          ${branchOptions}
  ...
  <button type="button" class="report-filter-clear" ${
    inline ? "" : 'id="report-filter-clear-all" '
