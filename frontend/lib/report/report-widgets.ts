...
function filterAttrSeverities(counts: SeverityCounts): string {
  ...
  return order.filter((s) => (counts[s] ?? 0) > 0).map((s) => filterAttr(s)).join(" ")
}
...
function buildTrendFilterPills(ctx: ReportContext, periodDays: number, countMode: string): string {
  const primary = ctx.branding?.primaryColor ?? "#0ea5e9"
  const pillClass = "trend-filter-pill"
  const daterangeLabel =
    periodDays <= 7 ? "Last 7 days" : periodDays <= 30 ? "Last 30 days" : periodDays <= 90 ? "Last 90 days" : periodDays <= 180 ? "Last 180 days" : "Last 365 days"
  const esc = (s: string) => s.replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/</g, "&lt;")
  const issuesPill = `<div class="${pillClass}" data-trend-dim="issues">
    <button type="button" class="trend-filter-pill-trigger" aria-haspopup="listbox">${countMode === "instances" ? "Individual Issues" : "Grouped Issues"} <span class="trend-filter-arrow">▾</span></button>
  ...
  const severityPill = `<div class="${pillClass}" data-trend-dim="severity">
    ...
    <div class="trend-filter-pill-panel trend-filter-panel-checkboxes" data-panel="severity">
      ...
      ${severityOptions.map((o) => `<label class="trend-filter-checkbox"><input type="checkbox" value="${esc(o)}" checked data-trend-severity> ${esc(o)}</label>`).join("")}
    </div>
  </div>`
  ...
  const typesPill = `<div class="${pillClass}" data-trend-dim="types">
    ...
    <div class="trend-filter-pill-panel trend-filter-panel-checkboxes" data-panel="types">
      ...
      ${typeOptions.map((o) => `<label class="trend-filter-checkbox"><input type="checkbox" value="${esc(o)}" checked data-trend-type> ${esc(o)}</label>`).join("")}
    </div>
  </div>`
...
function findMatchingContainer(repo: string | undefined, containerRisk: { repo: string }[]): string | null {
...
function sectionFilterAttrs(ctx: ReportContext): string {
  ...
  const branches = [...new Set(ctx.filteredIssues.map((i) => i.branch).filter(Boolean))]
  const containers = [...new Set(ctx.containerRisk.map((c) => c.repo).filter(Boolean))]
  const parts: string[] = []
  if (sev) parts.push(`data-filter-severity="${sev}"`)
  if (repos.length > 0) parts.push(`data-filter-repo="${repos.map((r) => filterAttr(r)).join(REPO_BRANCH_SEP)}"`)
  if (branches.length > 0) parts.push(`data-filter-branch="${branches.map((b) => filterAttr(b)).join(REPO_BRANCH_SEP)}"`)
  if (containers.length > 0) parts.push(`data-filter-container="${containers.map((c) => filterAttr(c)).join(REPO_BRANCH_SEP)}"`)
...
function svgSeverityDonut(counts: SeverityCounts, total: number, size = 100, primaryColor = "#0ea5e9"): string {
  const cx = size / 2
  const r = size / 2 - 4
  if (total === 0) return `<svg viewBox="0 0 ${size} ${size}" class="viz-donut" width="${size}" height="${size}"><circle cx="${cx}" cy="${cx}" r="${r}" fill="#f1f5f9"/><text x="${cx}" y="${cx + 4}" text-anchor="middle" font-size="12" fill="#64748b">0</text></svg>`
  ...
  const segments = keys
    .map((k, i) => {
      ...
      const pct = cnt / total
      const dash = pct * 100
      const seg = `<circle cx="${cx}" cy="${cx}" r="${r}" fill="none" stroke="${colors[i]}" stroke-width="8" pathLength="100" stroke-dasharray="${dash} ${100 - dash}" stroke-dashoffset="${-offset}" transform="rotate(-90 ${cx} ${cx})"/>`
      offset += dash
      return seg
    })
    .join("")
  return `<svg viewBox="0 0 ${size} ${size}" class="viz-donut" width="${size}" height="${size}"><circle cx="${cx}" cy="${cx}" r="${r}" fill="none" stroke="#e2e8f0" stroke-width="8" pathLength="100"/>${segments}<text x="${cx}" y="${cx + 5}" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">${total}</text></svg>`
}
...
function svgRiskGauge(score: number, size = 120): string {
  ...
  const cy = size / 2
  const r = size / 2 - 6
  const fillColor = score >= 75 ? "#ef4444" : score >= 50 ? "#f97316" : score >= 25 ? "#eab308" : "#22c55e"
  const pct = Math.min(100, Math.max(0, score)) / 100
  return `<svg viewBox="0 0 ${size} ${size}" class="viz-gauge" width="${size}" height="${size}"><path d="M 6 ${cy} A ${r} ${r} 0 0 1 ${size - 6} ${cy}" fill="none" stroke="#e2e8f0" stroke-width="8" pathLength="100"/><path d="M 6 ${cy} A ${r} ${r} 0 0 1 ${size - 6} ${cy}" fill="none" stroke="${fillColor}" stroke-width="8" pathLength="100" stroke-dasharray="${pct * 100} 100" stroke-linecap="round"/><text x="${cx}" y="${cy + 4}" text-anchor="middle" font-size="18" font-weight="700" fill="${fillColor}">${score}</text></svg>`
}

function svgTrendSparkline(trends: TrendDataPoint[], w = 280, h = 56, primaryColor = "#0ea5e9"): string {
...
function svgAgingBars(aging: AgingBucket[], maxW = 200, primaryColor = "#0ea5e9"): string {
  const totalMax = Math.max(1, ...aging.map((b) => b.critical + b.high + b.medium + b.low))
  return aging
    .map((b) => {
      ...
      const sevAttr = sevs.length > 0 ? ` data-filter-severity="${filterAttr(sevs.join(" "))}"` : ""
      return `<div class="aging-bar-row"${sevAttr}><span class="aging-bar-label">${b.range}</span><div class="aging-bar-track"><div class="aging-bar-fill" style="width:${w}px;background:linear-gradient(90deg,#ef4444 ${(b.critical / total) * 100}%,#f97316 ${(b.critical / total) * 100}% ${((b.critical + b.high) / total) * 100}%,#eab308 ${((b.critical + b.high) / total) * 100}% ${((b.critical + b.high + b.medium) / total) * 100}%,${primaryColor} ${((b.critical + b.high + b.medium) / total) * 100}%)"></div></div><span class="aging-bar-num">${total}</span></div>`
    })
    .join("")
}

function svgRepoRiskBars(repos: RepoRiskScore[], maxScore = 100, maxRepos = 8): string {
  ...
  return top
    .map((repo) => {
      ...
      const sevAttr = sevs.length > 0 ? ` data-filter-severity="${filterAttr(sevs.join(" "))}"` : ""
      return `<div class="repo-bar-row report-filterable" data-filter-repo="${filterAttr(repo.repo)}" data-filter-asset-type="${filterAttr("Code")}"${sevAttr}><span class="repo-bar-label mono" title="${repo.repo}">${repo.repo.length > 24 ? repo.repo.slice(0, 21) + "…" : repo.repo}</span><div class="repo-bar-track"><div class="repo-bar-fill ${repo.critical > 0 ? "repo-bar-critical" : repo.high > 0 ? "repo-bar-high" : ""}" style="width:${w}px"></div></div><span class="repo-bar-num">${repo.score}</span></div>`
    })
    .join("")
}

function svgContainerRiskBars(containers: ContainerRiskScore[], maxScore = 100, maxContainers = 8): string {
  ...
  return top
    .map((c) => {
      ...
      return `<div class="repo-bar-row report-filterable" data-filter-repo="${filterAttr(c.repo)}" data-filter-asset-type="${filterAttr("Container")}"${containerAttr}${sevAttr}><span class="repo-bar-label mono" title="${c.repo}">${c.repo.length > 24 ? c.repo.slice(0, 21) + "…" : c.repo}</span><div class="repo-bar-track"><div class="repo-bar-fill ${c.critical > 0 ? "repo-bar-critical" : c.high > 0 ? "repo-bar-high" : ""}" style="width:${w}px"></div></div><span class="repo-bar-num">${c.score}</span></div>`
    })
    .join("")
}

function svgAssetMixDonut(mix: AssetMix, total: number, size = 100, primaryColor = "#0ea5e9"): string {
  const cx = size / 2
  const r = size / 2 - 4
  if (total === 0)
    return `<svg viewBox="0 0 ${size} ${size}" class="viz-donut" width="${size}" height="${size}"><circle cx="${cx}" cy="${cx}" r="${r}" fill="#f1f5f9"/><text x="${cx}" y="${cx + 4}" text-anchor="middle" font-size="12" fill="#64748b">0</text></svg>`
  const segments = [
  ...
  const circles = segments
    .map((s) => {
      ...
      const seg = `<circle cx="${cx}" cy="${cx}" r="${r}" fill="none" stroke="${s.color}" stroke-width="8" pathLength="100" stroke-dasharray="${dash} ${100 - dash}" stroke-dashoffset="${-offset}" transform="rotate(-90 ${cx} ${cx})"/>`
      offset += dash
      return seg
    })
    .join("")
  return `<svg viewBox="0 0 ${size} ${size}" class="viz-donut" width="${size}" height="${size}"><circle cx="${cx}" cy="${cx}" r="${r}" fill="none" stroke="#e2e8f0" stroke-width="8" pathLength="100"/>${circles}<text x="${cx}" y="${cx + 5}" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">${total}</text></svg>`
}

const SCANNER_DONUT_COLORS = ["#0ea5e9", "#8b5cf6", "#ec4899", "#f97316", "#22c55e", "#64748b", "#eab308", "#94a3b8"]

function svgScannerDonut(scanners: ScannerBreakdown[], total: number, size = 100): string {
  const cx = size / 2
  const r = size / 2 - 4
  ...
  const segments = scanners
    .map((s, i) => {
      ...
      const seg = `<circle cx="${cx}" cy="${cx}" r="${r}" fill="none" stroke="${color}" stroke-width="8" pathLength="100" stroke-dasharray="${dash} ${100 - dash}" stroke-dashoffset="${-offset}" transform="rotate(-90 ${cx} ${cx})"/>`
      offset += dash
      return seg
    })
    .join("")
...
const TREND_STACKED_COLORS = ["#ef4444", "#f97316", "#3b82f6", "#14b8a6"] as const
const TREND_STACKED_KEYS: (keyof Omit<TrendDataPoint, "date" | "total">)[] = ["critical", "high", "medium", "low"]
const TREND_STACKED_LABELS = ["critical", "high", "medium", "low"] as const

function niceMax(value: number): number {
  if (value <= 0) return 100
  const step = value <= 10 ? 1 : value <= 50 ? 10 : value <= 200 ? 50 : value <= 500 ? 100 : 200
...
function svgTrendStacked(trends: TrendDataPoint[], w = 560, h = 160): string {
  ...
  for (let i = 0; i < n; i++) {
    ...
    TREND_STACKED_KEYS.forEach((k, ki) => {
      ...
      const tooltipText = `${t.date}: ${label} ${cnt}`.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
      ...
      const padding = 10
      const tooltipW = Math.max(64, Math.ceil(tooltipText.length * approxCharWidth) + padding * 2)
  ...
  const axisLine = `<line x1="${pad.left}" y1="${xAxisY}" x2="${pad.left + chartW}" y2="${xAxisY}" stroke="#e2e8f0" stroke-width="1"/>`
  const yAxisLine = `<line x1="${pad.left + chartW}" y1="${pad.top}" x2="${pad.left + chartW}" y2="${xAxisY}" stroke="#e2e8f0" stroke-width="1"/>`
  const yTicks = [0, Math.ceil(yMax / 4), Math.ceil(yMax / 2), Math.ceil((3 * yMax) / 4), yMax].filter((v, i, a) => a.indexOf(v) === i)
  const yLabels = yTicks
    .map((v) => {
      const y = pad.top + chartH - (v / yMax) * chartH
      return `<text x="${pad.left + chartW + 6}" y="${y + 4}" text-anchor="start" font-size="10" fill="#64748b">${v}</text>`
    })
    .join("")
  const maxXLabels = 14
  const xLabelIndices =
  ...
  const xLabels = xLabelIndices
    .map((i) => {
      const t = trends[i]
      const x = pad.left + i * (barW + gap) + barW / 2
      return `<text x="${x}" y="${h - 6}" text-anchor="middle" font-size="9" fill="#64748b">${t.date}</text>`
    })
    .join("")
  const legendX = 10
  const legend = TREND_STACKED_LABELS.map((label, i) => {
    const ly = pad.top - 2 + i * 14
    return `<circle cx="${legendX}" cy="${ly}" r="4" fill="${TREND_STACKED_COLORS[i]}"/><text x="${legendX + 10}" y="${ly + 4}" text-anchor="start" font-size="10" fill="#475569">${label}</text>`
  }).join("")
  const trendLegendLy = pad.top - 2 + 4 * 14
  const trendLegend =
    n >= 2
      ? `<line x1="${legendX}" y1="${trendLegendLy}" x2="${legendX + 12}" y2="${trendLegendLy}" stroke="${TREND_LINE_COLOR}" stroke-width="2" stroke-dasharray="4 2"/><text x="${legendX + 16}" y="${trendLegendLy + 4}" text-anchor="start" font-size="10" fill="#475569">Trend</text>`
      : ""
  const hoverCss = segIds
    .map((id) => `.viz-trend-stacked-container:has(.viz-trend-stacked [data-segment="${id}"]:hover) .viz-trend-stacked-overlay [data-for="${id}"]{opacity:1}`)
    .join("")
  const tooltipLayer = `<g class="trend-tooltip-layer">${tooltips.join("")}</g>`
  const regressionLine = linearRegressionLine(trends, yMax, pad, chartW, chartH, barW, gap)
  return `<div class="viz-trend-stacked-container" style="position:relative;width:100%;height:100%;min-height:140px"><svg viewBox="0 0 ${w} ${h}" class="viz-trend-stacked" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">${legend}${trendLegend}${axisLine}${yAxisLine}${barRects.join("")}${regressionLine}${yLabels}${xLabels}</svg><svg viewBox="0 0 ${w} ${h}" class="viz-trend-stacked-overlay" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style="position:absolute;top:0;left:0;pointer-events:none"><defs><style>.trend-segment-tooltip{opacity:0;transition:opacity .03s ease-out}${hoverCss}</style></defs>${tooltipLayer}</svg></div>`
}
...
function trendBadgeHtml(
  ...
  if (!change || change.direction === "flat" || change.pctChange === 0) return ""
  const arrow = change.direction === "up" ? "↑" : "↓"
  const pct = Math.abs(change.pctChange)
  const isGood = improvementIsUp ? change.direction === "up" : change.direction === "down"
...
function renderSummary(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const filterNote =
    filterParts.length > 0
      ? `<p class="filter-note">Filtered to: <strong>${filterParts.join(", ")}</strong></p>`
      : ""
  const notes = ctx.notes ? `<div class="notes"><strong>Notes:</strong> ${ctx.notes}</div>` : ""
  ...
  const cards = [
    ...
    kpiCard(
      ctx,
      "Avg MTTR",
      ctx.avgMttr !== undefined && Number.isFinite(ctx.avgMttr) ? String(ctx.avgMttr) : "—",
  ...
  if (variant === "weekly") {
    const trendDir =
      ctx.trends.length >= 2 && ctx.trends[ctx.trends.length - 1].total !== ctx.trends[ctx.trends.length - 2].total
        ? ctx.trends[ctx.trends.length - 1].total < ctx.trends[ctx.trends.length - 2].total
  ...
  if (variant === "compliance") {
    const processNarrative = ctx.avgMttr !== undefined ? `Mean time to remediate is ${ctx.avgMttr} days.` : "Remediation metrics are being collected."
...
function renderSeverityDonut(ctx: ReportContext, config: Record<string, unknown>): string {
  const size = Number(config.size) || 100
  /* Aggregate widget: always visible, content updated by client-side filter script */
  return `<div class="section" data-report-aggregate="severity-distribution"><div><h2>Severity Distribution</h2><div class="severity-donut-wrap">${svgSeverityDonut(ctx.counts, ctx.openIssues, size, getPrimaryColor(ctx))}</div></div></div>`
}
    ...
    ctx.openIssues > 0
      ? ["critical", "high", "medium", "low"]
        .map((s) => {
          ...
          return cnt > 0 ? `<div style="width:${(cnt / ctx.openIssues) * 100}%;background:${sevColor(s, primary)}">${cnt}</div>` : ""
        })
        .join("")
      : '<div style="width:100%;background:#e2e8f0;color:#94a3b8">No open issues</div>'
  /* Aggregate widget: always visible, content updated by client-side filter script */
  return `<div class="section" data-report-aggregate="severity-distribution"><div><h2>Severity Distribution</h2><div class="severity-with-donut"><div class="severity-donut-wrap">${svgSeverityDonut(ctx.counts, ctx.openIssues, 100, primary)}</div><div class="severity-bar-legend"><div class="severity-bar">${bar}</div><div class="severity-legend"><span><span class="dot" style="background:#ef4444"></span> Critical: ${ctx.counts.critical}</span><span><span class="dot" style="background:#f97316"></span> High: ${ctx.counts.high}</span><span><span class="dot" style="background:#eab308"></span> Medium: ${ctx.counts.medium}</span><span><span class="dot" style="background:${primary}"></span> Low: ${ctx.counts.low}</span></div></div></div></div></div>`
}

function renderSeverityPills(ctx: ReportContext): string {
  const primary = getPrimaryColor(ctx)
  return `<div class="section severity-compact" data-report-aggregate="severity-pills"><h2>By severity</h2><div class="severity-inline"><span class="sev-pill" data-filter-severity="${filterAttr("critical")}" data-severity="critical" style="background:#ef4444;color:#fff">Critical ${ctx.counts.critical}</span><span class="sev-pill" data-filter-severity="${filterAttr("high")}" data-severity="high" style="background:#f97316;color:#fff">High ${ctx.counts.high}</span><span class="sev-pill" data-filter-severity="${filterAttr("medium")}" data-severity="medium" style="background:#eab308;color:#fff">Medium ${ctx.counts.medium}</span><span class="sev-pill" data-filter-severity="${filterAttr("low")}" data-severity="low" style="background:${primary};color:#fff">Low ${ctx.counts.low}</span></div></div>`
}
...
function renderSourceBar(ctx: ReportContext): string {
  ...
  if (total === 0 || scanners.length === 0)
    return `<div class="section" data-report-aggregate="source-distribution"><div${attrs}><h2>Source Distribution</h2><div class="severity-with-donut"><div class="severity-donut-wrap">${svgScannerDonut([], 0, 100)}</div><div class="severity-bar-legend"><div class="severity-bar"><div style="width:100%;background:#e2e8f0;color:#94a3b8">No open issues</div></div><div class="severity-legend"><span>No sources</span></div></div></div></div>`
  const bar =
    scanners
      .slice(0, 8)
      .map((s, i) => {
      ...
      .join("") || '<div style="width:100%;background:#e2e8f0;color:#94a3b8">No open issues</div>'
  const legend = scanners
    .slice(0, 8)
    .map(
      (s, i) =>
        `<span><span class="dot" style="background:${SCANNER_DONUT_COLORS[i % SCANNER_DONUT_COLORS.length]}"></span> ${displaySourceName(s.scanner) || s.scanner}: ${s.count}</span>`
    )
    .join("")
  return `<div class="section" data-report-aggregate="source-distribution"><div${attrs}><h2>Source Distribution</h2><div class="severity-with-donut"><div class="severity-donut-wrap">${svgScannerDonut(scanners, total, 100)}</div><div class="severity-bar-legend"><div class="severity-bar">${bar}</div><div class="severity-legend">${legend}</div></div></div></div></div>`
}

function renderRiskGauge(ctx: ReportContext, config: Record<string, unknown>): string {
...
function renderTrendSparkline(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const attrs = sectionFilterAttrs(ctx)
  return `<div class="section"><div${attrs}><h2>12-Month Trend</h2><div class="viz-trend-wrap">${svgTrendSparkline(ctx.trends, w, h, getPrimaryColor(ctx))}</div></div></div>`
}

function renderTrendTable(ctx: ReportContext, config: Record<string, unknown>): string {
  if (ctx.trends.length === 0) return ""
  ...
  const rows = trends
    .map((t) => {
      ...
      const sevAttr = sevs.length > 0 ? ` data-filter-severity="${filterAttr(sevs.join(" "))}"` : ""
      return `<tr${sevAttr}><td>${t.date}</td><td class="text-right mono ${t.critical > 0 ? "critical-val" : ""}">${t.critical}</td><td class="text-right mono ${t.high > 0 ? "high-val" : ""}">${t.high}</td><td class="text-right mono ${t.medium > 0 ? "medium-val" : ""}">${t.medium}</td><td class="text-right mono ${t.low > 0 ? "low-val" : ""}">${t.low}</td><td class="text-right mono" style="font-weight:600">${t.total}</td></tr>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
  const totalNote = ctx.trends.length > trends.length ? ` (${trends.length} of ${ctx.trends.length})` : ""
...
function renderAgingBars(ctx: ReportContext, config: Record<string, unknown>): string {
  const maxW = Number(config.maxWidth) || 200
  const html = ctx.aging.length > 0 ? `<div class="viz-aging-bars">${svgAgingBars(ctx.aging, maxW, getPrimaryColor(ctx))}</div>` : ""
  const attrs = sectionFilterAttrs(ctx)
  return `<div class="section"><div${attrs}><h2>Vulnerability Aging</h2>${html}</div></div>`
}

function renderAgingTable(ctx: ReportContext, config: Record<string, unknown>): string {
  const limit = Number(config.limit) ?? 999
  ...
  const rows = aging
    ...
    .join("")
  const totalNote = ctx.aging.length > aging.length ? ` (${aging.length} of ${ctx.aging.length})` : ""
...
function renderMttrBars(ctx: ReportContext): string {
  ...
  const bars = ctx.mttr
    .map((m) => {
      ...
      return `<div class="mttr-bar-row" data-filter-severity="${filterAttr(s)}"><span class="badge ${sev(s)}">${m.severity}</span><div class="mttr-bar-track"><div class="mttr-bar-fill" style="width:${w}px;background:${sevColor(m.severity, primary)}"></div></div><span class="mttr-bar-num">${m.avgDays}d</span></div>`
    })
    .join("")
  const sevAttrs = ctx.mttr.map((m) => m.severity.toLowerCase()).map((s) => filterAttr(s)).join(" ")
...
function renderMttrTable(ctx: ReportContext): string {
  ...
  const rows = ctx.mttr
    .map((m) => {
      const s = m.severity.toLowerCase()
      return `<tr data-filter-severity="${filterAttr(s)}"><td><span class="badge ${sev(s)}">${m.severity}</span></td><td class="text-right mono">${m.avgDays}</td><td class="text-right mono">${m.medianDays}</td><td class="text-right mono">${m.count}</td></tr>`
    })
    .join("")
  const sevAttrs = ctx.mttr.map((m) => m.severity.toLowerCase()).map((s) => filterAttr(s)).join(" ")
  const mttrFilter = sevAttrs ? ` data-filter-severity="${sevAttrs}"` : ""
...
function renderRepoBars(ctx: ReportContext, config: Record<string, unknown>): string {
  const maxRepos = Number(config.maxRepos) || 8
  const repos = ctx.repoRisk.slice(0, maxRepos)
  if (repos.length === 0) return ""
  const attrs = sectionFilterAttrs(ctx)
  return `<div class="section"><div${attrs}><h2>Repository Risk</h2><div class="viz-repo-bars">${svgRepoRiskBars(repos, 100, maxRepos)}</div></div></div>`
}

function renderRepoTable(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const rows = repos
    .map((repo) => {
      ...
      if (repo.low > 0) sevs.push("low")
      const sevAttr = sevs.length > 0 ? ` data-filter-severity="${filterAttr(sevs.join(" "))}"` : ""
      return `<tr data-filter-repo="${filterAttr(repo.repo)}" data-filter-asset-type="${filterAttr("Code")}"${sevAttr}><td class="mono">${repo.repo}</td><td class="text-right mono ${repo.critical > 0 ? "critical-val" : ""}">${repo.critical || "-"}</td><td class="text-right mono ${repo.high > 0 ? "high-val" : ""}">${repo.high || "-"}</td><td class="text-right mono ${repo.medium > 0 ? "medium-val" : ""}">${repo.medium || "-"}</td><td class="text-right mono ${repo.low > 0 ? "low-val" : ""}">${repo.low || "-"}</td><td class="text-right mono" style="font-weight:600">${repo.score}</td></tr>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
...
function renderTopVulnsTable(ctx: ReportContext, config: Record<string, unknown>): string {
  const limit = Number(config.limit) ?? 25
  const vulns = ctx.topVulns.slice(0, limit)
  const issueById = (id: number) => ctx.filteredIssues.find((i) => i.issue_id === id)
    ...
    .map((v) => {
      ...
      const trackingCell =
        tasks && tasks.length > 0
          ? formatTaskLinks(tasks, ctx.external) || '<span style="color:#22c55e">Tracked</span>'
          : v.hasTask
            ? '<span style="color:#22c55e">Tracked</span>'
            : '<span style="color:#94a3b8">-</span>'
      const issueUrl = buildAikidoIssueUrl(ctx, issue?.repository ?? v.repo, issue?.issue_group_id ?? 0, issue?.issue_id ?? v.id) || v.aikidoUrl
      const titleCell = issueUrl
        ? aikidoIssueLink(issueUrl, v.title)
        : v.title.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
      const containerMatch = findMatchingContainer(v.repo, ctx.containerRisk)
      const containerAttr = containerMatch ? ` data-filter-container="${filterAttr(containerMatch)}"` : ""
      const assetType = getAssetTypeForIssue(v.repo, repoNames, containerNames, vmNames, packageNames)
      const assetTypeAttr = ` data-filter-asset-type="${filterAttr(assetType)}"`
      return `<tr data-filter-severity="${filterAttr(v.severity?.toLowerCase())}" data-filter-repo="${filterAttr(v.repo)}" data-filter-branch="${filterAttr(issue?.branch)}"${assetTypeAttr}${containerAttr}><td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${titleCell}</td><td><span class="badge ${sev(v.severity)}">${v.severity}</span></td><td class="mono">${v.cve}</td><td class="mono">${v.repo}</td><td class="text-right mono ${v.age > 90 ? "critical-val" : v.age > 30 ? "high-val" : ""}">${v.age}d</td><td>${trackingCell}</td></tr>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
...
function renderTopVulnsList(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const items = vulns
    .map((v) => {
      const cvss = Number.isFinite(v.score) && v.score > 0 ? ` · CVSS ${v.score.toFixed(1)}` : ""
      const issue = issueById(v.id)
      const issueUrl = buildAikidoIssueUrl(ctx, issue?.repository ?? v.repo, issue?.issue_group_id ?? 0, issue?.issue_id ?? v.id) || v.aikidoUrl
      const titlePart = issueUrl ? aikidoIssueLink(issueUrl, v.title) : v.title.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
      const containerMatch = findMatchingContainer(v.repo, ctx.containerRisk)
      const containerAttr = containerMatch ? ` data-filter-container="${filterAttr(containerMatch)}"` : ""
      const assetType = getAssetTypeForIssue(v.repo, repoNames, containerNames, vmNames, packageNames)
      const assetTypeAttr = ` data-filter-asset-type="${filterAttr(assetType)}"`
      return `<li data-filter-severity="${filterAttr(v.severity?.toLowerCase())}" data-filter-repo="${filterAttr(v.repo)}" data-filter-branch="${filterAttr(issue?.branch)}"${assetTypeAttr}${containerAttr}><span class="badge ${sev(v.severity)}">${v.severity}</span> ${titlePart} <span class="findings-meta">${v.cve !== "N/A" ? v.cve : ""}${cvss} · ${v.repo}</span></li>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
...
function aikidoIssueLink(url: string, text: string): string {
  if (!url) return text
  const escaped = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
  return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="aikido-link">${escaped}</a>`
}

/** Resolve Aikido repo ID from repo name with flexible matching (exact, lowercase, suffix, affected_repos). */
function resolveRepoId(
  ctx: ReportContext,
  repoName: string | undefined,
  issueGroupId: number
): number | undefined {
  const map = ctx.repoIdByName
  if (!map) return undefined

  const tryLookup = (name: string): number | undefined => {
    if (!name) return undefined
    const id = map[name] ?? map[name.toLowerCase()]
    if (id !== undefined) return id
    if (name.includes("/")) {
      const lastSeg = name.split("/").pop()
      if (lastSeg) return map[lastSeg] ?? map[lastSeg.toLowerCase()]
    }
    for (const [key, id] of Object.entries(map)) {
      if (key === name || key.toLowerCase() === name.toLowerCase()) return id
      if (key.endsWith("/" + name)) return id
    }
    return undefined
  }

  const fromRepo = repoName ? tryLookup(repoName) : undefined
  if (fromRepo !== undefined) return fromRepo

  const groups = ctx.issueGroups ?? []
  const group = groups.find((g) => g.group_id === issueGroupId)
  if (group?.affected_repos?.length) {
    for (const ar of group.affected_repos) {
      const id = tryLookup(ar)
      if (id !== undefined) return id
    }
  }
  return undefined
...
function buildAikidoIssueUrl(
  ...
  const base = ctx.aikidoBaseUrl
  if (!base || !issueGroupId) return ""
  const repoId = resolveRepoId(ctx, repoName, issueGroupId)
  if (!repoId) return ""
  return `${base}/repositories/${repoId}?sidebarIssue=${issueGroupId}`
}

function normalizeCveKey(cve: string): string {
  if (!cve || cve === "N/A") return ""
  return `CVE-${String(cve).replace(/^CVE-?/i, "").toUpperCase()}`
}

function renderTopVulnsAdvisory(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const cards = vulns
    .map((v) => {
      ...
      const fixed = issue?.fixed_version ? `Update to <code>${issue.fixed_version}</code> or apply vendor patch.` : "No fix version reported; apply mitigation or monitor vendor advisory."
      const cwe = issue?.cwe_id ? `CWE-${String(issue.cwe_id).replace("CWE-", "")}` : ""
      const cveDisplay = v.cve && v.cve !== "N/A" ? cveLink(v.cve) : v.cve || "—"
      const pkgRow = issue?.affected_package || issue?.affected_version ? `<div class="advisory-row"><strong>Affected</strong> ${issue?.affected_package || ""} ${issue?.affected_version || ""}</div>` : ""
      const tasks = ctx.tasksByGroupId?.[issue?.issue_group_id ?? 0]
      const tasksRow = tasks && tasks.length > 0 ? `<div class="advisory-row"><strong>Linked tasks</strong> ${formatTaskLinks(tasks, ctx.external)}</div>` : ""
      const issueUrl = buildAikidoIssueUrl(ctx, issue?.repository ?? v.repo, issue?.issue_group_id ?? 0, issue?.issue_id ?? v.id) || v.aikidoUrl
      const titlePart = issueUrl ? aikidoIssueLink(issueUrl, v.title) : v.title.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
      const linkRow = issueUrl ? `<div class="advisory-row"><strong>View in Aikido</strong> ${aikidoIssueLink(issueUrl, "Open issue")}</div>` : ""
      ...
      const epssRow =
        cveDetails && (cveDetails.epss_score !== undefined || cveDetails.in_kev)
          ? `<div class="advisory-row"><strong>Risk</strong>${cveDetails.epss_score !== undefined ? ` EPSS ${(cveDetails.epss_score * 100).toFixed(2)}%` : ""}${cveDetails.in_kev ? ' · <span class="badge badge-critical" title="In CISA Known Exploited Vulnerabilities catalog">KEV</span>' : ""}</div>`
          : ""
      const containerMatch = findMatchingContainer(v.repo, ctx.containerRisk)
      const containerAttr = containerMatch ? ` data-filter-container="${filterAttr(containerMatch)}"` : ""
      const assetType = getAssetTypeForIssue(v.repo, repoNames, containerNames, vmNames, packageNames)
      const assetTypeAttr = ` data-filter-asset-type="${filterAttr(assetType)}"`
      return `<div class="advisory-card report-filterable" data-filter-severity="${filterAttr(v.severity?.toLowerCase())}" data-filter-repo="${filterAttr(v.repo)}" data-filter-branch="${filterAttr(issue?.branch)}"${assetTypeAttr}${containerAttr}><div class="advisory-header"><span class="badge ${sev(v.severity)}">${v.severity}</span><span class="advisory-title">${titlePart}</span></div><div class="advisory-row"><strong>ID / CVE</strong> ${cveDisplay}${cwe ? ` · ${cwe}` : ""}</div><div class="advisory-row"><strong>Description</strong> ${desc}</div><div class="advisory-row"><strong>Impact</strong> ${v.severity === "critical" || v.severity === "high" ? "High impact; may allow unauthorized access or compromise." : "Medium or low impact depending on context."}</div><div class="advisory-row"><strong>CVSS</strong> ${Number.isFinite(v.score) && v.score > 0 ? v.score.toFixed(1) : "—"} ${v.repo ? ` · Affected: ${v.repo}` : ""}</div>${epssRow}${pkgRow}${tasksRow}${linkRow}<div class="advisory-row"><strong>Resolution</strong> ${fixed}</div></div>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
...
function renderScannerTable(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const rows = scanners
    .map((s) => {
      ...
      const sevAttr = sevs.length > 0 ? ` data-filter-severity="${filterAttr(sevs.join(" "))}"` : ""
      return `<tr${sevAttr}><td class="mono">${displaySourceName(s.scanner) || s.scanner}</td><td class="text-right mono" style="font-weight:600">${s.count}</td><td class="text-right mono ${s.critical > 0 ? "critical-val" : ""}">${s.critical || "-"}</td><td class="text-right mono ${s.high > 0 ? "high-val" : ""}">${s.high || "-"}</td><td class="text-right mono ${s.medium > 0 ? "medium-val" : ""}">${s.medium || "-"}</td><td class="text-right mono ${s.low > 0 ? "low-val" : ""}">${s.low || "-"}</td></tr>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
  const totalNote = ctx.scanners.length > scanners.length ? ` (${scanners.length} of ${ctx.scanners.length})` : ""
...
function renderScannerDonut(ctx: ReportContext, config: Record<string, unknown>): string {
  if (ctx.scanners.length === 0) return ""
  ...
  const legend = ctx.scanners
    ...
    .join("")
  const attrs = sectionFilterAttrs(ctx)
  return `<div class="section"><div${attrs}><h2>Findings by Scanner</h2><div class="severity-with-donut"><div class="severity-donut-wrap">${svgScannerDonut(ctx.scanners, total, size)}</div><div class="severity-bar-legend severity-legend">${legend}</div></div></div></div>`
}

function renderTrendStacked(ctx: ReportContext, config: Record<string, unknown>): string {
  const periodDays = (ctx.effectivePeriodDays ?? Number(config.periodDays)) || 90
  ...
  const content = trends.length > 0 ? svgTrendStacked(trends, w, h) : `<div class="viz-trend-empty" style="display:flex;align-items:center;justify-content:center;min-height:140px;color:#64748b;font-size:14px">No findings in selected period</div>`
  const trendFilter = ""
  ...
  const openPct =
    metrics.openOneWeekAgo === 0
      ? currentOpen > 0
        ...
        : 0
      : Math.round(
        ((currentOpen - metrics.openOneWeekAgo) / metrics.openOneWeekAgo) * 100
      )
  const resolvedPct =
  ...
  const newPct =
    metrics.newLastWeek === 0
      ? metrics.newThisWeek > 0
        ...
        : 0
      : Math.round(
        ((metrics.newThisWeek - metrics.newLastWeek) / metrics.newLastWeek) * 100
      )

  const openPctText = openPct > 0 ? `+${openPct}%` : openPct < 0 ? `${openPct}%` : "0%"
  const resolvedPctText =
    resolvedPct > 0 ? `+${resolvedPct}%` : resolvedPct < 0 ? `${resolvedPct}%` : "0%"
  const newPctText = newPct > 0 ? `+${newPct}%` : newPct < 0 ? `${newPct}%` : "0%"

  const openPctColor = currentOpen <= metrics.openOneWeekAgo ? "#22c55e" : "#ef4444"
  const resolvedPctColor =
    metrics.resolvedThisWeek >= metrics.resolvedLastWeek ? "#22c55e" : "#ef4444"
  const newPctColor = metrics.newThisWeek <= metrics.newLastWeek ? "#22c55e" : "#ef4444"
...
function renderCriticalHighKpi(ctx: ReportContext): string {
  const criticalHigh = ctx.counts.critical + ctx.counts.high
  const attrs = sectionFilterAttrs(ctx)
  return `<div class="section"><div${attrs}><h2>Priority Findings</h2><div class="kpi-grid" style="grid-template-columns: 1fr;"><div class="kpi-card"><div class="label">Critical &amp; High</div><div class="value" style="color:${criticalHigh > 0 ? "#f97316" : "#22c55e"}">${criticalHigh}</div><div class="detail">${criticalHigh > 0 ? "Requires attention" : "None open"}</div></div></div></div></div>`
}
...
function renderIssueList(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const rows = issues
    .map((i) => {
      const age = Math.max(0, Math.round((Date.now() - new Date(i.first_detected_at).getTime()) / 86400000))
      const s = (i.severity ?? "").toLowerCase()
      const tasks = ctx.tasksByGroupId?.[i.issue_group_id ?? 0]
      const tasksCell = tasks && tasks.length > 0 ? formatTaskLinks(tasks, ctx.external) : "-"
      const aikidoUrl = buildAikidoIssueUrl(ctx, i.repository, i.issue_group_id ?? 0, i.issue_id)
      const idCell = aikidoUrl ? aikidoIssueLink(aikidoUrl, String(i.issue_id)) : String(i.issue_id)
      const titleCell = aikidoUrl ? aikidoIssueLink(aikidoUrl, i.title) : i.title.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
      const containerMatch = findMatchingContainer(i.repository, ctx.containerRisk)
      const containerAttr = containerMatch ? ` data-filter-container="${filterAttr(containerMatch)}"` : ""
      const sortStatus = isOpen(i) ? "open" : "closed"
      const source = displaySourceName(i.scanner_type) || "-"
      const assetType = getAssetTypeForIssue(i.repository, repoNames, containerNames, vmNames, packageNames)
      const asset = i.repository || "-"
      const assetTypeAttr = ` data-filter-asset-type="${filterAttr(assetType)}"`
      return `<tr data-filter-severity="${filterAttr(s)}" data-filter-repo="${filterAttr(i.repository)}" data-filter-branch="${filterAttr(i.branch)}" data-sort-status="${sortStatus}"${assetTypeAttr}${containerAttr}><td class="mono">${idCell}</td><td>${titleCell}</td><td><span class="badge ${sev(s)}">${i.severity}</span></td><td class="mono">${source}</td><td>${assetType}</td><td class="mono">${asset}</td><td class="text-right mono">${Number.isFinite(age) ? `${age}d` : "-"}</td><td>${i.status}</td><td>${tasksCell}</td></tr>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
...
function renderContainerBars(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  return `<div class="section"><div${attrs}><h2>Container Risk</h2><div class="viz-repo-bars">${svgContainerRiskBars(containers, 100, maxContainers)}</div></div></div>`
}

function renderContainerTable(ctx: ReportContext, config: Record<string, unknown>): string {
  const limit = Number(config.limit) ?? 25
  ...
  const rows = containers
    .map((c) => {
      ...
      const sevAttr = sevs.length > 0 ? ` data-filter-severity="${filterAttr(sevs.join(" "))}"` : ""
      const containerAttr = ` data-filter-container="${filterAttr(c.repo)}"`
      return `<tr data-filter-repo="${filterAttr(c.repo)}" data-filter-asset-type="${filterAttr("Container")}"${containerAttr}${sevAttr}><td class="mono">${c.repo}</td><td class="text-right mono ${c.critical > 0 ? "critical-val" : ""}">${c.critical || "-"}</td><td class="text-right mono ${c.high > 0 ? "high-val" : ""}">${c.high || "-"}</td><td class="text-right mono ${c.medium > 0 ? "medium-val" : ""}">${c.medium || "-"}</td><td class="text-right mono ${c.low > 0 ? "low-val" : ""}">${c.low || "-"}</td><td class="text-right mono" style="font-weight:600">${c.score}</td></tr>`
    })
    .join("")
  const attrs = sectionFilterAttrs(ctx)
...
function renderAssetMixDonut(ctx: ReportContext, config: Record<string, unknown>): string {
  const mix = ctx.assetMix
  const total = mix.code + mix.container + mix.vm + (mix.package ?? 0) + mix.other
  const attrs = sectionFilterAttrs(ctx)
  const size = Number(config.size) ?? 100
  const primary = getPrimaryColor(ctx)
  if (total === 0)
  ...
  const legend = [
    ...
    .join("")
  return `<div class="section" data-report-aggregate="asset-mix" data-asset-mix-size="${size}"><div${attrs}><h2>Issues by Asset Type</h2><div class="asset-mix-body"><div class="severity-with-donut"><div class="severity-donut-wrap">${svgAssetMixDonut(mix, total, size, getPrimaryColor(ctx))}</div><div class="severity-bar-legend severity-legend">${legend}</div></div></div></div></div>`
}

function renderTeamTable(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const rows = limited
    ...
    .join("")
  const totalNote = teams.length > limited.length ? ` (${limited.length} of ${teams.length})` : ""
...
function renderActivityTimeline(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const rows = items
    .map((a) => {
      ...
      return `<tr><td class="mono" style="font-size:11px;color:#64748b">${date}</td><td>${a.action}${target}${user}</td>${a.details ? `<td style="font-size:11px;color:#475569">${a.details}</td>` : "<td></td>"}</tr>`
    })
    .join("")
...
function renderText(ctx: ReportContext, config: Record<string, unknown>): string {
...
function renderComplianceScoreCard(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const status = overview.status ?? (score >= 80 ? "Compliant" : score >= 50 ? "Partial" : "At risk")
  const controls =
    overview.controls_total != null
      ? `${overview.controls_passed ?? 0}/${overview.controls_total} controls passed`
      : ""
...
function renderReachabilityMatrix(ctx: ReportContext): string {
  ...
  const trs = rows
    .map((r) => {
      ...
      return `<tr data-filter-severity="${filterAttr(s)}"><td><span class="badge ${sev(s)}">${r.severity}</span></td><td class="text-right mono" style="color:#22c55e">${r.exploitable}</td><td class="text-right mono" style="color:#64748b">${r.notExploitable}</td><td class="text-right mono" style="color:#94a3b8">${r.unknown}</td><td class="text-right mono" style="font-weight:600">${r.exploitable + r.notExploitable + r.unknown}</td></tr>`
    })
    .join("")
...
function renderCiScanFrequency(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const recent = scans.filter((s) => new Date(s.created_at ?? s.timestamp ?? 0).getTime() >= cutoff)
  ...
  const rows = sorted
    .map(
      ([date, d]) =>
        `<tr><td class="mono">${date}</td><td class="text-right mono">${d.total}</td><td class="text-right mono" style="color:#22c55e">${d.success}</td><td class="text-right mono" style="color:#64748b">${d.total - d.success}</td></tr>`
    )
    .join("")
...
function renderABCCompliance(ctx: ReportContext, _config: Record<string, unknown>): string {
  ...
  const attrs = sectionFilterAttrs(ctx)
  const tooltipAttr = ` data-tooltip="${ABC_INFO_TITLE.replace(/"/g, "&quot;")}"`
  if (!abc) {
  ...
  const issues: string[] = []
  if (abc.maxCountExceeded.critical) issues.push("Critical max count (1) exceeded")
  if (abc.maxCountExceeded.high) issues.push("High max count (4) exceeded")
  if (abc.justificationOverdue > 0) issues.push(`${abc.justificationOverdue} justification(s) overdue`)
  if (abc.remediationOverdue > 0) issues.push(`${abc.remediationOverdue} remediation(s) overdue`)
  if (abc.cveAgeExceeded > 0) issues.push(`${abc.cveAgeExceeded} CVE age tolerance exceeded`)
  const issuesHtml = issues.length > 0 ? `<ul class="abc-issues">${issues.map((i) => `<li>${i}</li>`).join("")}</ul>` : ""
  return `<div class="section"><div${attrs}><h2${tooltipAttr}>ABC Compliance</h2><p class="narrative-line">Acceptance Baseline Criteria</p><div class="kpi-grid" style="grid-template-columns: repeat(2, 1fr);"><div class="kpi-card"><div class="label">Status</div><div class="value" style="color:${statusColor}">${abc.compliant ? "Compliant" : "Non-compliant"}</div></div><div class="kpi-card"><div class="label">Findings</div><div class="value">${abc.justificationOverdue + abc.remediationOverdue + abc.cveAgeExceeded}</div><div class="detail">overdue / exceeded</div></div></div>${issuesHtml}</div></div>`
}

function renderSlaCompliance(ctx: ReportContext, config: Record<string, unknown>): string {
  ...
  const statusColor = sla.pctCompliant >= 80 ? "#22c55e" : sla.pctCompliant >= 50 ? "#eab308" : "#ef4444"
  const rows = sla.bySeverity
    .map((r) => {
      const s = r.severity.toLowerCase()
      return `<tr data-filter-severity="${filterAttr(s)}"><td><span class="badge ${sev(s)}">${r.severity}</span></td><td class="text-right mono" style="color:#22c55e">${r.within}</td><td class="text-right mono" style="color:#ef4444">${r.exceeding}</td></tr>`
    })
    .join("")
...
function renderCvssHeatmap(ctx: ReportContext, config: Record<string, unknown>): string {
  const periodDays = (ctx.effectivePeriodDays ?? Number(config.periodDays)) || 90
  const w = Number(config.width) || 560
  const h = Number(config.height) || 200
  const countMode = ctx.countMode ?? "groups"
  const trends = computeTrendDataLastDays(ctx.filteredIssues, periodDays, countMode)
  const attrs = sectionFilterAttrs(ctx)
  if (trends.length === 0) {
  ...
  for (let ri = 0; ri < 4; ri++) {
    ...
    for (let ci = 0; ci < trends.length; ci++) {
      ...
      cells.push(
        `<rect x="${x}" y="${y}" width="${cellW - 2}" height="${cellH - 2}" fill="${heatColors[ri]}" opacity="${opacity}" rx="2"/><text x="${x + cellW / 2 - 4}" y="${y + cellH / 2 + 4}" text-anchor="middle" font-size="10" fill="${intensity > 0.5 ? "#fff" : "#334155"}">${label}</text>`
      )
    }
  }
  const maxXLabels = 14
  ...
  const xLabelIndices =
    nTrends <= maxXLabels
      ...
      : Array.from({ length: maxXLabels }, (_, k) => Math.round((k * (nTrends - 1)) / (maxXLabels - 1)))
  const xLabels = xLabelIndices
    .map((i) => {
      const t = trends[i]
      const x = pad.left + (i + 0.5) * cellW
      return `<text x="${x}" y="${h - 8}" text-anchor="middle" font-size="9" fill="#64748b">${t.date}</text>`
    })
    .join("")
  const yLabels = rows
    .map((r, i) => {
      const y = pad.top + (i + 0.5) * cellH
      return `<text x="8" y="${y + 4}" text-anchor="start" font-size="10" fill="#64748b">${r}</text>`
    })
    .join("")
  return `<div class="section"><div${attrs}><h2>Severity Trend Heatmap</h2><p class="narrative-line">Vulnerability density by severity over time (last ${periodDays} days)</p><div class="viz-trend-wrap"><svg viewBox="0 0 ${w} ${h}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">${cells.join("")}${xLabels}${yLabels}</svg></div></div></div>`
}
...
function formatTaskLinks(
  ...
  return tasks
    .map((t) => {
      ...
      if (t.url && !noLinks) return `<a href="${t.url}" target="_blank" rel="noopener noreferrer" class="task-link">${label}</a>`
