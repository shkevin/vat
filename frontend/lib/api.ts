...
export async function fetchFindings(
  ...
  if (params?.status) search.set("status", Array.isArray(params.status) ? params.status.join(",") : params.status);
  if (params?.severity) search.set("severity", Array.isArray(params.severity) ? params.severity.join(",") : params.severity);
  if (params?.source) search.set("source", Array.isArray(params.source) ? params.source.join(",") : params.source);
  if (params?.type) search.set("type", Array.isArray(params.type) ? params.type.join(",") : params.type);
  if (params?.asset) search.set("asset", params.asset);
  if (params?.search) search.set("search", params.search);
  if (params?.search_fields) {
    const v = Array.isArray(params.search_fields) ? params.search_fields.join(",") : params.search_fields;
    search.set("search_fields", v);
  }
  if (params?.limit !== undefined) search.set("limit", String(params.limit));

  const url = `${API_BASE}/findings${search.toString() ? `?${search}` : ""}`;
  const res = await vatFetch(url, { headers: authHeaders(auth?.token, auth?.userEmail) }, auth);
  if (!res.ok) {
...
export async function updateFinding(
  ...
  if (data.compensatingControls != null) body.compensating_controls = data.compensatingControls;
  if (data.reviewerNote != null) body.reviewer_note = data.reviewerNote;
  if (data.suppressionScope != null) body.suppression_scope = data.suppressionScope;
...
export async function unarchiveFinding(id: string, auth?: Auth): Promise<ApiFinding> {
  const res = await vatFetch(
...
export async function fetchIntegrationSchemas(auth?: Auth): Promise<IntegrationSchemas> {
  const res = await vatFetch(
...
export async function fetchParsers(auth?: Auth): Promise<{
  parsers: Array<{ id: string; name: string; label: string; description?: string }>;
}> {
  const res = await vatFetch(
...
export async function pullAikidoData(
  ...
  auth?: Auth
): Promise<{ message: string; fetched: number; created: number; merged: number }> {
  const params = new URLSearchParams();
...
export async function syncAikido(auth?: Auth, sourceId?: string | null): Promise<{
  status: string;
  message: string;
}> {
  const res = await vatFetch(
    ...
    {
      method: "POST",
      headers: { ...authHeaders(auth?.token, auth?.userEmail), "Content-Type": "application/json" },
...
export async function regenerateIngestKey(
  ...
  const res = await vatFetch(
    `${API_BASE}/settings/ingest-keys/${encodeURIComponent(sourceId)}/regenerate`,
    { method: "POST", headers: authHeaders(auth?.token, auth?.userEmail) },
    auth
  );
  if (!res.ok) throw new Error(`API error: ${res.status} ${res.statusText}`);
...
export async function createOAuthClient(
  sourceId: string,
  auth?: Auth
): Promise<{ sourceId: string; clientId: string; clientSecret: string; message: string }> {
  const res = await vatFetch(
...
export async function downloadExportBundle(auth?: Auth): Promise<void> {
  ...
  const filename = match?.[1] ?? `vat-export-${new Date().toISOString().slice(0, 10)}.zip`;
...
export async function fetchSbomPackages(
  ...
  auth?: Auth
): Promise<Array<{ id: string; name: string; version: string; licenseId?: string; licenseRisk?: string; component?: string; language?: string }>> {
  const params = new URLSearchParams();
...
export async function downloadSbomPackages(
  ...
  const disposition = res.headers.get("Content-Disposition");
  const match = disposition?.match(/filename="?([^";\n]+)"?/);
  const filename = match?.[1] ?? `sbom-${options.component ?? "all"}-${new Date().toISOString().slice(0, 10)}.${options.format ?? "csv"}`;
  const blob = await res.blob();
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = filename;
...
export function apiHeaders(token?: string | null, userEmail?: string | null): HeadersInit {
...
function authHeaders(token?: string | null, userEmail?: string | null): HeadersInit {
  if (token) return { Authorization: `Bearer ${token}` };
  return userEmail ? { "X-VAT-User": userEmail } : {};
}

export async function fetchTenants(auth?: Auth): Promise<Array<{ id: string; name: string; auth_method?: string }>> {
  const res = await vatFetch(
    `${API_BASE}/tenants`,
    { headers: apiHeaders(auth?.token, auth?.userEmail) },
    auth
...
export async function fetchUsers(auth?: Auth, tenantId?: string): Promise<
  Array<{ id: string; tenant_id: string | null; email: string; role: string }>
> {
  const url = tenantId ? `${API_BASE}/users?tenant_id=${encodeURIComponent(tenantId)}` : `${API_BASE}/users`;
  const res = await vatFetch(url, { headers: apiHeaders(auth?.token, auth?.userEmail) }, auth);
  if (!res.ok) throw new Error(`API error: ${res.status} ${res.statusText}`);
  return res.json();
}
...
export async function createUser(
  ...
  auth?: Auth
): Promise<{ id: string; tenant_id: string | null; email: string; role: string }> {
  const res = await vatFetch(
    `${API_BASE}/users`,
    {
      method: "POST",
...
export async function deleteTenant(tenantId: string, auth?: Auth): Promise<void> {
  const res = await vatFetch(
    `${API_BASE}/tenants/${encodeURIComponent(tenantId)}`,
    { method: "DELETE", headers: apiHeaders(auth?.token, auth?.userEmail) },
    auth
